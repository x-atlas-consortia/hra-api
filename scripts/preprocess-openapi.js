...
const isString = (node) => isScalar(node) && typeof node.value === 'string';

const isRef = (node) => isMap(node) && isString(node.get('$ref', true));

const getRef = (node) => (isRef(node) ? node.get('$ref') : undefined);
...
const matchesKey = (node, key) => isPair(node) && isString(node.key) && node.key.value === key;
